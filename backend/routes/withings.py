...
@router.get("/api/withings/oauth-state")
async def get_withings_oauth_state(user: User = Depends(get_current_user)):
    """Mints a one-time nonce binding the upcoming Withings authorization to the caller's account."""
    return {"state": generate_oauth_state(user.id)}

@router.get("/api/withings/callback", response_class=HTMLResponse)
async def get_withings_callback(
    request: Request,
    code: str = Query("", description="Authorization code"),
    state: str = Query("", description="OAuth state nonce from /api/withings/oauth-state")
):
    code = code.strip()
    if not code:
    ...
    try:
        client_id = get_api_key('freja_withings_client_id', user_id=user_id) or ""
        client_secret = get_api_key('freja_withings_client_secret', user_id=user_id) or ""
        ...
        set_api_key('freja_withings_refresh_token', new_refresh_token, user_id=user_id)
