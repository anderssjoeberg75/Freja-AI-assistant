...
FrejaUIController.prototype.bindEvents = function () {
    ...
    if (btnSyncGarminDashboard) {
        btnSyncGarminDashboard.addEventListener('click', async () => {
            soundSynth.playClick();
            self.writeLog("INITIATING GARMIN DEVICE SYNCHRONIZATION (SINCE LAST SYNC)", "sys");
            try {
    ...
    if (btnTrainer && modalTrainer && btnCloseTrainer) {
        ...
        // Dropdown menu switcher for PT Dashboard (Overview, Daily Check-In, Garmin, Withings Hälsa, Settings)
        const trainerViewSelect = document.getElementById('trainer-view-select');
