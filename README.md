...
* 🗣️ **Freja Knows Your Program in Ordinary Chat**: Every chat turn injects the live program — active plan, this week's booked sessions with today's marked, active injuries and your real training load — into her system prompt from `GET /api/trainer/context`. She answers "hur ser dagens träningspass ut" from the actual schedule rather than depending on a tool call landing, and can no longer improvise a session that is nowhere in the plan.
