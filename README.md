...
* 🛟 **Syncs Fail Loudly, Never Destructively**: A sync that cannot reach its provider reports an error instead of quietly substituting demo data, and never reports success for a run that did not happen. Strava's "overwrite" re-sync replaces history in a single transaction, so an expired token or a rate limit leaves the existing activities untouched rather than wiping them. The Google Calendar cleanup pass only prunes events inside the window it actually fetched, so PT sessions booked further ahead are not deleted locally while still live in Google.
* 🕳️ **A Missing Reading Stays Missing**: A day the watch recorded nothing is stored as NULL, never as `0` and never as a copy of the previous day. Health averages divide by the number of days that actually carry a reading, so a week with unworn days no longer reports a resting heart rate well below the real one, and the HUD shows `–` rather than inventing a value.
* 📆 **Catch-Up Sync After a Long Break**: A Garmin sync fetches at most 30 days per run to stay clear of rate limits, but the days a longer gap leaves uncovered are queued rather than dropped — each later sync drains another chunk, oldest first, until the history is whole. Previously the last-sync marker jumped to now regardless, so anything the cap trimmed became a permanent hole.
* 🌐 **Only Your Own Machine and LAN May Read the API**: CORS is no longer `*`. Loopback and private/LAN origins are trusted — that is what a self-hosted HUD on `:5000` talking to a backend on `:8000` actually is — and any public origin must be opted into via `freja_allowed_origins`. One shared policy in [origins.py](backend/origins.py) answers this for CORS, for the headers on auth failures, and for OAuth redirect targets, so the three cannot drift apart.
* 🔑 **The Access Token Goes Only to Your Backend**: The client's fetch wrapper decides where the master credential is sent by comparing URL *origins*, not string prefixes, so a protocol-relative or look-alike host (`//other/api/x`, `your.ip.evil.com`, a different port) never receives it.
...
* 🎯 **Guided Onboarding**: The PT panel's **ONBOARDING** button hands the whole profile over to Freja. She first analyses 90 days of Garmin, Strava and Withings data (`POST /api/trainer/onboarding/start`) and derives everything the devices already know — real weekly volume, typical and longest sessions per activity, heart-rate ranges, resting HR / sleep / HRV baselines, body composition — then interviews you about only what the data *cannot* show: your actual goal, your real schedule, gym access, preferred activities, and pain not yet logged. Each question comes pre-filled with her own data-based answer, so accepting her reading takes one click. `POST /api/trainer/onboarding/complete` merges your answers with the measurements (your intent wins for goals and limits; the measured baselines win over any guess) and writes every field of the training profile.
* 📅 **Progression Built on a Month of Real Training**: Plan generation reads the last 30 days of *completed* training (Strava, with Garmin filling in sessions Strava never saw), and passes the coach per-week volume, per-activity typical and longest durations, plus hard minute ceilings: no single session may exceed the longest recent one by more than 20%, and weekly volume may not rise more than 10%. This is what stops a plan from jumping from a habitual 20-minute jog to an hour-long run — if the goal genuinely needs more, the coach lays out the build-up over several weeks instead.
* 🗣️ **Freja Knows Your Program in Ordinary Chat**: The PT panel no longer carries its own chat box — it was a second, weaker chat with a separate history and no tool access. Every chat turn injects the live program — active plan, this week's booked sessions with today's marked, active injuries and your real training load — into her system prompt from `GET /api/trainer/context`. She answers "hur ser dagens träningspass ut" from the actual schedule rather than depending on a tool call landing, and can no longer improvise a session that is nowhere in the plan.
...
* 📈 **Trend & Adherence Charts**: The PT panel plots resting HR and HRV over a selectable 14/28/90-day window as dependency-free inline SVG sparklines — each with a dashed reference line for your stored baseline — plus a planned-vs-completed adherence bar. Served in one request by `GET /api/trainer/trends`. Days a device recorded nothing render as gaps, never as a dip to zero.
* 🩹 **Injury & Pain Log**: Track niggles over time (date, body area, severity 1–10, note) instead of a single free-text limitations field, via `/api/trainer/injuries`. Active entries are fed into plan generation, the daily check-in and the recovery optimizer, so affected sessions are automatically eased or swapped; resolving an entry keeps it as history but drops it out of the coach's prompts.
* ☀️ **Daily Check-in with Visible Briefing**: The PT panel's **CHECK IN** button (`POST /api/trainer/checkin`) *first* pulls the freshest data from Garmin, Strava and Withings — all three sync concurrently with a small recent-days window, capped so a stalled provider can't hold up the briefing, and every failure or missing credential is non-fatal (the check-in still briefs from whatever is stored). It then reads last night's Garmin/Withings snapshot, checks whether yesterday's session was completed on Strava, weighs in today's calendar and the weather, and asks the coach for a short Swedish morning briefing. If recovery warrants it and a session is booked today, F.R.E.J.A. re-times that session in the calendar automatically. The coach's briefing — previously generated but never shown — now renders as a card at the top of the **Briefing / Feedback** panel (formerly "Previous Recommendations"), with context badges for today's session, yesterday's completion, whether the session was auto-adjusted, and recent adherence, plus a data-freshness line reporting what each wearable sync did.
* 📤 **Plan Export (ICS / PDF)**: Take a saved plan out of Freja with `GET /api/trainer/plans/export?plan_id=…&format=ics|pdf`. The `.ics` file imports into any calendar app; the PDF is a printable summary. Both are generated by [plan_export.py](backend/services/plan_export.py) with no third-party dependency, and both schedule the plan exactly as the calendar booking does.
...
**Permission gateway** — the HUD's tool gateway in [ui-tools.js](client/js/ui-tools.js) reads the callable tool set from `GET /api/tools/metadata`, which the backend derives from the same registry. It used to keep its own hand-written whitelist of tool → permission key and refuse anything absent from it, so a tool added to the registry but forgotten in the client was unreachable from the web UI even though the backend could run it (this is what made `get_trainer_workouts` and the Instagram tools fail). The client map now holds display labels only; permission enforcement stays server-side in `is_tool_execution_authorized`.

**HUD proxy timeouts** — [run_client.py](run_client.py) proxies `/api/` to the backend, so its timeout silently caps how long any HUD request may take. Ordinary reads keep a short 30s limit (a dead backend should be noticed fast), while model-backed paths — onboarding, plan generation, the check-in, the optimizer, chat and tool execution — get 300s, comfortably longer than the backend's own `GEMINI_TIMEOUT_SECONDS`. A flat 30s used to cut Gemini off mid-generation and report "Bad Gateway", which pointed at the wrong component entirely; onboarding, which analyses 90 days of data first, could never finish. Proxy failures now answer with a FastAPI-shaped `{"detail": …}` body so the HUD shows the real reason instead of a bare status code.

**Plan booking anchor** — a plan's `day` field is a Swedish weekday name that booking turns into an absolute offset from the start date (Måndag = 0), so the start date must *be* a Monday. Generating a plan used to auto-book it from today, shifting every session by today's weekday — a Wednesday generation put "Måndag" on Wednesday. Auto-booking now anchors on `_current_week_monday()` and skips sessions that would land before today; an explicitly requested start date (`POST /api/trainer/plans/book`) is still honoured in full.

**Plan export** — [plan_export.py](backend/services/plan_export.py) turns a stored plan into an `.ics` calendar or a printable PDF without pulling in a PDF or calendar library: the ICS writer handles RFC 5545 escaping and 75-octet line folding, and the PDF is assembled object by object with its own xref table. The Swedish weekday→offset map lives there and is imported by the calendar booking, so an exported plan and a booked plan always land on the same dates.
